...
class _ProductDetailsScreenState extends State<ProductDetailsScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      create: (context) => ProductDetailsBloc(sl())..add(ProductDetailsEvent(widget.id)),
   ...
   return Scaffold(
       ...
       body:   SingleChildScrollView(
         child: Column(
           ...
           children: [
             DetailsAppBar(product: state.productDetailsEntities!.name),
             CarouselSlider.builder(
                 itemCount: state.productDetailsEntities!.images.length,
                 itemBuilder: (context, index, realIndex) {

                   return AppImage(
                       imageUrl: state.productDetailsEntities!.images[index],
                   ...
                   Row(
                     ...
                     children: [
                       ...
                       IconButton(onPressed: (){
                         /// add to fav
                         state.productDetailsEntities!.inFavorites = ! state.productDetailsEntities!.inFavorites;
setState(() {

});

                       }, icon: Icon(state.productDetailsEntities!.inFavorites == true ?Icons.favorite :Icons.favorite_outline,color: AppColors.primary,))
                     ],
                   ),
                   SizedBox(height: 2.h,),
                   Row(children: [
                     Text(
                       state.productDetailsEntities!.price.toString(),
                       style: TextStyle(
                         color: AppColors.primary,
                         fontSize: 18.sp,
                         fontWeight: FontWeight.bold,
                     ...
                     SizedBox(width: 3.w,),
                     Visibility(
                       visible: state.productDetailsEntities!.price.toString() != state.productDetailsEntities!.oldPrice.toString()  ,
                       child: Row(
                         children: [
                           Text(
                             state.productDetailsEntities!.oldPrice.toString(),
                             style: TextStyle(
                           ...
                           Container(
                               ...
                               BorderRadius.circular(10.sp),color: AppColors.primary.withOpacity(0.5)),

                               child:  Text( "${state.productDetailsEntities!.discount}%",
